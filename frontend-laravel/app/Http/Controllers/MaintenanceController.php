...
class MaintenanceController extends Controller
{
    ...
    private function updateApiData($endpoint, $data = [])
    {
        try {
            $response = Http::withToken(session('auth_token'))->put($this->apiUrl() . $endpoint, $data);
            return [
                'ok' => $response->successful(),
                'message' => $response->json('message') ?? ($response->successful() ? 'Berhasil' : 'Request gagal'),
            ];
    ...
    private function destroyApiData($endpoint)
    {
        try {
            $response = Http::withToken(session('auth_token'))->delete($this->apiUrl() . $endpoint);
            return [
                'ok' => $response->successful(),
                'message' => $response->json('message') ?? ($response->successful() ? 'Berhasil' : 'Request gagal'),
            ];
    ...
    public function update(Request $request, $id)
    {
        ...
        $result = $this->updateApiData("/maintenance/logs/{$id}", $payload);
        return $result['ok']
            ? back()->with('success', $result['message'])
            : back()->withInput()->with('error', $result['message']);
    }
    ...
    {
        $result = $this->destroyApiData("/maintenance/logs/{$id}");
        return $result['ok']
            ? back()->with('success', $result['message'])
