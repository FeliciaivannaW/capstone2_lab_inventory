                        ...
                        <tr id="draft-row-{{ $draft['id'] }}" class="hover:bg-slate-50/50 transition-all duration-300">
                            ...
                            <td class="text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    @php
                                        $canSubmit = ($draft['item_count'] ?? 0) > 0 && ($draft['rejected_count'] ?? 0) == 0;
                                    @endphp
                                    <button type="button" 
                                        @if($canSubmit) @click="openSubmitModal({{ $draft['id'] }})" @endif
                                        :disabled="submitting === {{ $draft['id'] }} || !{{ $canSubmit ? 'true' : 'false' }}" 
                                        class="inline-flex items-center gap-1.5 px-3 py-1.5 text-white text-xs font-semibold rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed {{ $canSubmit ? 'bg-amber-500 hover:bg-amber-600' : 'bg-slate-300' }}"
                                        @if(!$canSubmit) title="Draft tidak valid (Kosong atau terdapat item ditolak)" @endif>
