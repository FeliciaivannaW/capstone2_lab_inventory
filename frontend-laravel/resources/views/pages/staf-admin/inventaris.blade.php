...
<div class="flex items-center justify-between mb-2 flex-wrap gap-3">
    <div>
        ...
        <p class="text-sm text-slate-500 mt-0.5">Pantau seluruh aset dan BHP secara menyeluruh (Read-Only).</p>
    </div>
...
<div class="flex items-center gap-6 border-b border-slate-200 mb-6">
    <a href="{{ route('staf-admin.inventaris', ['tab' => 'aset']) }}" 
       class="py-3 px-1 text-sm font-semibold border-b-2 transition-colors {{ $tab === 'aset' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300' }}">
        Aset Inventaris
    </a>
    <a href="{{ route('staf-admin.inventaris', ['tab' => 'bhp']) }}" 
       class="py-3 px-1 text-sm font-semibold border-b-2 transition-colors {{ $tab === 'bhp' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300' }}">
...
<form method="GET" action="{{ route('staf-admin.inventaris') }}"
    ...
    <div class="flex-[2] min-w-[180px]">
        <label class="text-[0.68rem] font-semibold text-slate-400 uppercase tracking-wider mb-1.5 block">Cari BHP</label>
        <input type="text" name="search" value="{{ $filters['search'] ?? '' }}"
    ...
    <div class="flex gap-2">
        ...
        @if(array_filter($filters))
            <a href="{{ route('staf-admin.inventaris', ['tab' => 'bhp']) }}" class="inline-flex items-center px-3 py-2 text-sm font-semibold text-slate-500 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors">Reset</a>
        @endif
    </div>
</form>

{{-- TABLE BHP --}}
<div class="glass-card rounded-2xl overflow-hidden" x-data="inventarisApp({{ count($bhpStocks) }})">
    <div class="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between">
        <p class="text-sm font-semibold text-slate-700">
            {{ count($bhpStocks) }} BHP ditemukan
        </p>
    </div>
    ...
    @if(empty($bhpStocks))
        <div class="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p class="text-sm font-semibold text-slate-500">Tidak ada BHP ditemukan</p>
            <p class="text-xs text-slate-400 mt-1">Coba ubah filter pencarian Anda.</p>
        </div>
    @else
        <div class="overflow-x-auto">
            <table class="w-full text-sm lv-table">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        <th class="px-4 py-3 text-left text-[10px] font-bold text-slate-400 uppercase tracking-wider w-8">#</th>
                        <th class="px-4 py-3 text-left text-[10px] font-bold text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors" @click="sortBy('item_name', $el)" field="item_name">
                            <div class="flex items-center gap-1.5">Nama BHP <svg class="w-3 h-3 text-slate-300" x-show="sortField !== 'item_name'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4"/></svg> <svg class="w-3 h-3 text-indigo-500" x-show="sortField === 'item_name' && sortAsc" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/></svg> <svg class="w-3 h-3 text-indigo-500" x-show="sortField === 'item_name' && !sortAsc" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg></div>
                        </th>
                        <th class="px-4 py-3 text-left text-[10px] font-bold text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors" @click="sortBy('lab_name', $el)" field="lab_name">
                            <div class="flex items-center gap-1.5">Laboratorium <svg class="w-3 h-3 text-slate-300" x-show="sortField !== 'lab_name'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4"/></svg> <svg class="w-3 h-3 text-indigo-500" x-show="sortField === 'lab_name' && sortAsc" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/></svg> <svg class="w-3 h-3 text-indigo-500" x-show="sortField === 'lab_name' && !sortAsc" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg></div>
                        </th>
                        <th class="px-4 py-3 text-left text-[10px] font-bold text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors" @click="sortBy('current_stock', $el)" field="current_stock">
                            <div class="flex items-center gap-1.5">Sisa Stok <svg class="w-3 h-3 text-slate-300" x-show="sortField !== 'current_stock'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4"/></svg> <svg class="w-3 h-3 text-indigo-500" x-show="sortField === 'current_stock' && sortAsc" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/></svg> <svg class="w-3 h-3 text-indigo-500" x-show="sortField === 'current_stock' && !sortAsc" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg></div>
                        </th>
                        <th class="px-4 py-3 text-left text-[10px] font-bold text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors" @click="sortBy('status', $el)" field="status">
                            <div class="flex items-center gap-1.5">Status <svg class="w-3 h-3 text-slate-300" x-show="sortField !== 'status'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4"/></svg> <svg class="w-3 h-3 text-indigo-500" x-show="sortField === 'status' && sortAsc" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/></svg> <svg class="w-3 h-3 text-indigo-500" x-show="sortField === 'status' && !sortAsc" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg></div>
                        </th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                    @foreach($bhpStocks as $i => $bhp)
                    @php
                        $stock = (int)$bhp['current_stock'];
                        $min = (int)($bhp['minimum_stock'] ?? 0);
                        $st = $stock <= 0 ? 'Habis' : ($stock <= $min ? 'Menipis' : 'Aman');
                        $stColor = $st === 'Habis' ? 'bg-red-100 text-red-700 border-red-200' : ($st === 'Menipis' ? 'bg-amber-100 text-amber-700 border-amber-200' : 'bg-emerald-100 text-emerald-700 border-emerald-200');
                        $stDot = $st === 'Habis' ? 'bg-red-500' : ($st === 'Menipis' ? 'bg-amber-500' : 'bg-emerald-500');
                    @endphp
                    <tr x-show="showRow({{ $i }})" x-cloak class="hover:bg-slate-50 transition-colors">
                        <td class="px-4 py-3.5 text-slate-400 font-mono text-xs">{{ $i + 1 }}</td>
                        <td class="px-4 py-3.5 font-medium text-slate-800">{{ $bhp['item_name'] ?? '—' }}</td>
                        <td class="px-4 py-3.5 text-slate-600">{{ $bhp['laboratory_name'] ?? '—' }}</td>
                        <td class="px-4 py-3.5" data-sort-value="{{ $stock }}">
                            <span class="font-bold {{ $stock <= $min ? 'text-red-600' : 'text-slate-700' }}">{{ $stock }}</span> 
                            <span class="text-xs text-slate-500">{{ $bhp['unit'] ?? $bhp['catalog_unit'] ?? '' }}</span>
                            @if($min > 0)
                                <p class="text-[10px] text-slate-400 mt-0.5">Min: {{ $min }}</p>
                            @endif
                        </td>
                        <td class="px-4 py-3.5">
                            <span class="inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full border {{ $stColor }} whitespace-nowrap">
        ...
        <x-pagination :total="count($bhpStocks)" />
    @endif
</div>
@endif
