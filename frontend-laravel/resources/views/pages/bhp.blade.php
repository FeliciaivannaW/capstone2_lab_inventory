                                ...
                                <div class="rounded-xl border border-slate-100 p-4 hover:border-slate-200 transition-colors">
                                    <div class="flex items-center justify-between gap-2">
                                        <span class="text-xs font-bold px-2 py-1 rounded-md" 
                                              :class="movement.movement_type === 'in' ? 'bg-emerald-50 text-emerald-700' : (movement.movement_type === 'adjustment' ? 'bg-indigo-50 text-indigo-700' : 'bg-rose-50 text-rose-700')" 
                                              x-text="movementLabels[movement.movement_type] || movement.movement_type">
                                        </span>
                                        <span class="text-sm font-bold text-slate-700" 
                                              x-text="(movement.movement_type === 'in' ? '+' : (movement.movement_type === 'adjustment' ? '' : '-')) + movement.quantity + ' ' + riwayatStockUnit">
                                        </span>
                                    </div>
    ...
    @foreach($stocks as $stock)
        ...
        <template x-teleport="body">
            ...
            <div x-show="activeModal === 'stok_{{ $stock['id'] }}'"
                ...
                <div class="p-6">
                    <form action="{{ route('bhp.movement', $stock['id']) }}" method="POST" class="space-y-4" x-data="{ actionType: '' }">
                        @csrf
                        <div>
                            <label class="block text-xs font-semibold text-slate-500 mb-2">Jenis Aksi</label>
                            <div class="grid grid-cols-3 gap-2">
                                <label class="cursor-pointer">
                                    <input type="radio" name="movement_type" value="in" x-model="actionType" class="peer sr-only" required>
                                ...
                                <label class="cursor-pointer">
                                    <input type="radio" name="movement_type" value="out" x-model="actionType" class="peer sr-only" required>
                                ...
                                <label class="cursor-pointer">
                                    <input type="radio" name="movement_type" value="adjustment" x-model="actionType" class="peer sr-only" required>
                        ...
                        <div>
                            ...
                            <input type="number" :min="actionType === 'adjustment' ? 0 : 1" name="quantity" class="w-full rounded-xl border-slate-200 text-sm" placeholder="Masukkan jumlah" required>
                        </div>
